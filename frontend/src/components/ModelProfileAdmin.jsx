...
export default function ModelProfileAdmin() {
  ...
  return (
    <section className="admin-section">
      ...
      <Stack gap="sm">{error ? <Alert color="red" variant="light">{error}</Alert> : null}{notice ? <Alert color="teal" variant="light">{notice}</Alert> : null}</Stack>
      <Stack gap="lg">
        {groups.map((group) => {
          ...
          return <Paper key={group.scope} className="panel" withBorder radius="md" p="lg">
            ...
            {scoped.length === 0 ? <Alert color="blue" variant="light">尚无数据库默认 Profile；当前调用会使用非敏感的环境变量迁移兜底（如已配置）。页面不会读取或展示环境中的密钥。</Alert> : <Stack gap="sm">{scoped.map((profile) => <Paper key={profile.id} withBorder p="md" radius="md">
      ...
      <Modal opened={editing !== null} onClose={close} title={editing === 'new' ? '新增模型 Profile' : '编辑模型 Profile'} size="lg">
        <form onSubmit={save}><Stack>
          ...
          {testResult ? <Alert color={testResult.pending ? 'blue' : testResult.ok ? 'green' : 'red'} variant="light">{testResult.pending ? '正在测试连接…' : testResult.ok ? `连接成功，延迟 ${testResult.latencyMS}ms${testResult.model ? `，响应模型 ${testResult.model}` : ''}` : testResult.error}</Alert> : null}
          {testResult?.testsSavedVersion ? <Alert color="blue" variant="light">本次使用服务端已保存的 Base URL、模型 ID 与密钥进行测试。若要测试未保存的 URL 或模型变更，请同时输入用于测试的新 API Key，或先保存变更。</Alert> : null}
